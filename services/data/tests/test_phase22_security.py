...
def test_domain_is_database_network_environment_clock_random_subprocess_and_git_free() -> None:
    root = Path("services/data/src/fable5_data/phase22")
...
def test_builder_succeeds_while_network_entrypoints_are_denied(
    ...
    def deny(*args: object, **kwargs: object) -> None:
        ...
        raise AssertionError("Phase 22 attempted network access")
...
def test_ambient_environment_cannot_change_canonical_bytes(
    ...
    before = canonical_macro_vintage_candidate_inventory_amendment_bytes()
    monkeypatch.setenv("FABLE5_PHASE22_FAKE_CREDENTIAL", "phase22-secret-canary")
    monkeypatch.setenv("DATABASE_URL", "postgresql://phase22-secret-canary")
    monkeypatch.setenv("HTTP_PROXY", "http://phase22-secret-canary.invalid")
    after = canonical_macro_vintage_candidate_inventory_amendment_bytes()
    assert after == before
    assert b"phase22-secret-canary" not in after


def test_artifact_contains_no_credential_data_or_selection_payload() -> None:
    artifact = build_family_a_macro_vintage_candidate_inventory_amendment()
