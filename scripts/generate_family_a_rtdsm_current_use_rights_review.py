"""Generate the canonical Phase 23 RTDSM current-use-rights review."""
...
FAILURE_MESSAGE = "Family A RTDSM current-use-rights review generation failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Generate the deterministic Phase 23 public-terms-only RTDSM rights review without "
            "accessing data, granting rights, selecting a product, researching, or executing."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_public_terms_rights_review_only is not True:
            raise _InvalidInvocation
        from fable5_data.phase23.rights_review import (
            canonical_rtdsm_current_use_rights_review_bytes,
        )

        rendered = canonical_rtdsm_current_use_rights_review_bytes()
