"""Generate the sole canonical Phase 19 Step-3 prerequisite assessment."""
...
FAILURE_MESSAGE = "Family A Step-3 prerequisite assessment generation failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Generate deterministic Phase 19 Family A Step-3 prerequisite metadata. This creates "
            "no evaluation policy, holdout, credential, provider request, data, or qualification."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_prerequisite_assessment_only is not True:
            raise _InvalidInvocation
        from fable5_data.phase19.assessment import (
            canonical_step3_prerequisite_assessment_bytes,
        )

        rendered = canonical_step3_prerequisite_assessment_bytes()
