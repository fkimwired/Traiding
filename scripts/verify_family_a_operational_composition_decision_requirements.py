"""Verify one canonical Phase 21 decision-requirements artifact offline."""
...
FAILURE_MESSAGE = "Family A operational-composition decision-requirements verification failed.\n"
...
class _InvalidRequirements(Exception):
    pass
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description="Verify one deterministic Phase 21 decision-requirements artifact.",
        allow_abbrev=False,
    )
...
def _load_contract() -> tuple[Any, Any]:
    ...
    contracts = __import__(
        "fable5_data.phase21.contracts",
        fromlist=["FamilyAOperationalCompositionDecisionRequirements"],
    )
    builder = __import__(
        "fable5_data.phase21.decision_requirements",
        fromlist=["canonical_operational_composition_decision_requirements_bytes"],
    )
    ...
    if any(
        ...
        raise _InvalidRequirements
    model = getattr(contracts, "FamilyAOperationalCompositionDecisionRequirements", None)
    canonical_bytes = getattr(
        builder, "canonical_operational_composition_decision_requirements_bytes", None
    )
...
def _verify(path_text: str) -> dict[str, object]:
    try:
        ...
        raise _InvalidRequirements from None
    ...
    return {
        ...
        "capability_assignment_count": len(validated.capability_assignments),
        "candidate_group_count": len(validated.candidate_group_bindings),
        "decision_field_count": len(validated.decision_fields),
        "network": "disabled",
        "outcome": validated.outcome.value,
        "product_rights_binding_count": len(validated.product_rights_bindings),
