"""Generate the sole canonical Phase 20 evaluation/holdout input register."""
...
FAILURE_MESSAGE = "Family A evaluation/holdout input-register generation failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Generate deterministic Phase 20 Family A input requirements. This creates no "
            "operational selection, policy, holdout, credential, data, or qualification."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_input_register_only is not True:
            raise _InvalidInvocation
