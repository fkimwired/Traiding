"""Generate the deterministic Phase 26 operational data-composition decision."""
...
FAILURE_MESSAGE = "Phase 26 operational data-composition generation failed.\n"
...
class _SingleConfirmation(argparse.Action):
    def __call__(
        self,
...
def _parser() -> argparse.ArgumentParser:
    parser = _Parser(
        description="Generate the offline Phase 26 composition decision without provider access.",
        allow_abbrev=False,
    )
    parser.add_argument(
        "--confirm-operational-composition-decision-only",
        action=_SingleConfirmation,
        nargs=0,
        default=False,
        required=True,
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_operational_composition_decision_only is not True:
            raise _InvalidInvocation
        from fable5_data.phase26.composition import canonical_phase26_decision_bytes

        rendered = canonical_phase26_decision_bytes()
