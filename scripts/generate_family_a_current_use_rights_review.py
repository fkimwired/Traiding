"""Generate the sole canonical Phase 18 current-use rights review."""
...
FAILURE_MESSAGE = "Family A current-use rights review generation failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Generate deterministic Phase 18 Family A public-terms review metadata. This loads "
            "no credential, contract body, provider payload, account, or data."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_public_terms_review_only is not True:
            raise _InvalidInvocation
        from fable5_data.phase18.rights_review import canonical_current_use_rights_review_bytes

        rendered = canonical_current_use_rights_review_bytes()
