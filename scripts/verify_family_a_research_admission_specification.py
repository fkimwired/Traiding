"""Verify one canonical Phase 15 Family A specification completely offline."""
...
FAILURE_MESSAGE = "Family A research-admission specification verification failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Verify the deterministic Phase 15 Family A requirements-only specification "
...
def _read_specification(path_text: str) -> tuple[bytes, dict[str, object]]:
    path = Path(path_text)
    try:
        metadata = path.lstat()
        if not stat.S_ISREG(metadata.st_mode):
            raise _InvalidSpecification
        if metadata.st_size <= 0 or metadata.st_size > MAX_SPECIFICATION_BYTES:
            raise _InvalidSpecification
        raw = path.read_bytes()
    except _InvalidSpecification:
        raise
    except (OSError, OverflowError, ValueError):
        raise _InvalidSpecification from None
    if len(raw) != metadata.st_size or len(raw) > MAX_SPECIFICATION_BYTES:
        raise _InvalidSpecification
...
def _load_contract() -> tuple[Any, Any]:
    ...
    contracts = __import__(
        "fable5_data.phase15.contracts",
        fromlist=["FamilyAResearchAdmissionSpecification"],
    )
    specification = __import__(
        "fable5_data.phase15.specification",
        fromlist=["canonical_specification_bytes"],
    )
    ...
    if any(
        ...
        raise _InvalidSpecification
    model = getattr(contracts, "FamilyAResearchAdmissionSpecification", None)
    canonical_bytes = getattr(specification, "canonical_specification_bytes", None)
...
def _verify(path_text: str) -> dict[str, str]:
    raw, _decoded = _read_specification(path_text)
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        result = _verify(arguments.specification)
