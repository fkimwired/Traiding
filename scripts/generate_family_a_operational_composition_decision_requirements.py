"""Generate the sole canonical Phase 21 composition decision requirements."""
...
FAILURE_MESSAGE = "Family A operational-composition decision-requirements generation failed.\n"
...
def _parser() -> argparse.ArgumentParser:
    parser = _SanitizedArgumentParser(
        description=(
            "Generate deterministic Phase 21 decision requirements without selecting, ranking, "
            "valuing, submitting, requesting, persisting, researching, executing, or ordering."
        ),
        allow_abbrev=False,
    )
    parser.add_argument(
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if arguments.confirm_decision_requirements_only is not True:
            raise _InvalidInvocation
